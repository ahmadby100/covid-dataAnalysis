...
// Imports
import requests, {AxiosRequestConfig} from "axios";
...
export default class GitHubAPI {
  private baseUrl: string;
    ...
    getRepoDirectories = async (nested: boolean): Promise<Contents> => {
      ...
      const rootDirectories: Array<string> = [];
      ...
      const recurse = async (rootDirectories: Array<string>) => {
        ...
        for (const j in rootDirectories) {
          ...
          const nestedDirFiles: Files = [];
          const nestedDirDirectories: Array<string> = [];
